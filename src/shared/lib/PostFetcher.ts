...
export async function getPosts(): Promise<BlogPost[] | undefined> {
  const res = await fetch(
    `https://api.github.com/repos/joshhartwig/meblogposts/git/trees/main?recursive=1`,
    {
      headers: {
...
export async function getPostsByName(
  ...
  const res = await fetch(
    `https://raw.githubusercontent.com/joshhartwig/meblogposts/main/${filename}`,
    {
      headers: {
        Accept: 'application/vnd.github+json',
        Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
